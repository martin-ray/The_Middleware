...
int main() {
  ...
  std::cout << "loading dataset 1" << std::endl;
  ...
  //... load data into in_array_cpu4
  
  std::vector<mgard_x::SIZE> shape{n2, n3, n4};
  mgard_x::Config config;
  config.lossless = mgard_x::lossless_type::Huffman_LZ4;
  mgard_x::Hierarchy<3, float, mgard_x::CUDA> hierarchy(shape, config);
  mgard_x::Array<3, float, mgard_x::CUDA> in_array(shape);
  in_array.load(result2.data());
  std::cout << "Done\n";

  std::cout << "Compressing with MGARD-X CUDA backend..." << std::flush;
  float tol = 0.01, s = 0, norm;
  ...
  std::cout << "size: " << (float)64*n1*n2*n3/8/1024/1024/1024 << " GBytes" << std::endl;
  std::cout << "Throughput: " << (float)64*n1*n2*n3/8/1024/1024/1024/((float)duration/1000) << "Gbytes/sec" << std::endl;

  mgard_x::DeviceRuntime<mgard_x::CUDA>::SyncQueue(0);

  std::cout << "Decompressing with MGARD-X CUDA backend...";
  // decompression
  mgard_x::Array<3, float, mgard_x::CUDA> decompressed_array;
