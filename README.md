...
- [ ] 置換がうまくいっているか、目視テスト
- [ ] ストレージ読み出し速度の測定。これでHDF5 > TileDBだったら、めっちゃ悲しいぞ。
- [ ] HDF5 -> TileDBに変更。これでミドルウェア単体の性能を評価できる。
- [ ] TileDBフォーマットでデータを流し込む
- [ ] やっぱり、L3を入れると一気に性能が悪化する。なぜか？ユーザがミスした時の読み出し時間がめっちゃ長くなるから。0.3秒とかになる。プリフェッチしてユーザに取りに行かせなくて巣妙にしよう！という粋な計らいをしたL3君が、逆に邪魔になっているというね。
なので、L3はL4を介してのみリクエストできればいいってことなのか？どうなんだ、先にしたのを実装してみようか。
