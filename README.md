...
# 内部にベクトルを持っているやつだけど、
これ、別に難しいことをしているわけではないのです。
本当に、ただ、取ってくる順番を、時間方向を優先にしただけなのです。マジで。
マジでそんなに難しいことじゃない。提案手法としては、すごく簡単に拡張できる。うん、簡単だね。
その方向から順番に何個取ってくるかって話だからね。それだけの話。まじで！！
...
- [x] 多少は予測をしてプリフェッチをするようにしないといけない。ベクター保持して
...
サーバ側にリクエストを送っているか確認。
- [x] L1プリフェッチャがちゃんとL2にリクエストを出しているかを確認。
- [x] L2プリフェッチャのevictionがおかしい。ちゃんと追い出されているのか？？マジで。
